...
/**
 * 
 * This verticle basically receives {@link JsonObject} data instances from the eventbus and indexes it to the corresponding ES instance
 * 
 ...
 *     host
 *     port
 *     indexMode           {@link IndexMode}
 *     indexNameOrPattern
 *     ssl (true/false)
 *     sslTrustAll (true/false)
 *     authentication {
 *       type                (basic/aws)
...
public class ElasticSearchIndexerVerticle extends AbstractVerticle {
	...
	private void readConfig() {
		...
		if(this.config() == null 
		   || this.config().getJsonArray(Configuration.INSTANCES, null) == null) {
		...
		this.indexScheduleInterval = this.config().getLong(Configuration.INDEX_SCHEDULE_INTERVAL, Configuration.Defaults.INDEX_SCHEDULE_INTERVAL);
	...
	private ElasticSearchIndexerConfiguration readInstanceConfig(final Object xInstance) {
		...
		ElasticSearchIndexerConfiguration config = new ElasticSearchIndexerConfiguration().setIdentifier(identifier)
				.setHost(host).setIndexMode(indexMode).setIndexNameOrPattern(indexNameOrPattern).setPort(port.intValue());
		...
		if(authentication != null && "basic".equals(authentication.getString("type", null))) {
			// For now we only support basic so we can create the basic authentication implementation directly
	...
	public void start() throws Exception {
		...
		this.configurations.values().forEach(config -> {
			LOG.info("identifier [{}], host [{}], port[{}], indexMode[{}], indexNameOrPattern[{}], isSSL[{}], isSSLtrustAll[{}] ", config.getIdentifier(), config.getHost(), config.getPort(), config.getIndexMode(), config.getIndexNameOrPattern(), config.isSSL(), config.isSSLTrustAll() );
		});
	...
	private void indexCurrentData() {
		...
		Map<String, List<JsonObject>> valuesByIdentifier = drainedValues.parallelStream().collect(Collectors.groupingBy(value -> value.getJsonObject(ElasticSearchIndexerConstants.Message.Structure.Field.META.getFieldName()).getString(ElasticSearchIndexerConstants.Message.Structure.Field.INSTANCE_IDENTIFIER.getFieldName())));
		...
		for(final String identifier : valuesByIdentifier.keySet()) {
			...
			request
			  .sendBuffer(Buffer.buffer(indexString), ar -> {
			    if (ar.succeeded()) {
	...
	private void initializeClient() {
		...
		for (ElasticSearchIndexerConfiguration indexerConfig : this.configurations.values()) {
			...
			LOG.info("Initialized WebClient for identifier[{}] at [{}:{}] using SSL[{}] and trustAll[{}]", indexerConfig.getIdentifier(), indexerConfig.getHost(), indexerConfig.getPort(), indexerConfig.isSSL(), indexerConfig.isSSLTrustAll());
	...
	private HttpRequest<Buffer> getRequestFor(final ElasticSearchIndexerConfiguration indexerConfiguration){
		...
		HttpRequest<Buffer> request = webClient.post(indexerConfiguration.getPort(), indexerConfiguration.getHost(), "/_bulk");
	...
	private String getIndexPrefixString(final ElasticSearchIndexerConfiguration indexerConfiguration, final long eventTimestamp) {
		...
		if(IndexMode.DATE_PATTERN_EVENT_TIMESTAMP.equals(indexerConfiguration.getIndexMode())
			...
			long timestamp = IndexMode.DATE_PATTERN_EVENT_TIMESTAMP.equals(indexerConfiguration.getIndexMode()) ? eventTimestamp : System.currentTimeMillis();
			...
			if(!this.cachedDynamicIndexPrefix.containsKey(cacheKey)) {
				...
				ZonedDateTime tsDateTime = ZonedDateTime.ofInstant(Instant.ofEpochMilli(timestamp), ZoneId.of(TIMEZONE_UTC));
				...
				String formattedIndexPrefix = String.format("{ \"index\" : { \"_index\" : \"%s\", \"_type\" : \"%s\" } }%s",  formattedIndexPattern, "_doc", this.newLine);
			...
			if(this.cachedStaticIndexPrefix == null) {
				...
				this.cachedStaticIndexPrefix = String.format("{ \"index\" : { \"_index\" : \"%s\", \"_type\" : \"%s\" } }%s",  indexerConfiguration.getIndexNameOrPattern(), indexerConfiguration.getType(), this.newLine);
	...
	private String getIndexString(final ElasticSearchIndexerConfiguration indexerConfiguration, final Collection<JsonObject> values) {
		...
		for(JsonObject value : values) {
			...
			sb.append(getIndexPrefixString(indexerConfiguration, value.getJsonObject(ElasticSearchIndexerConstants.Message.Structure.Field.META.getFieldName()).getLong(ElasticSearchIndexerConstants.Message.Structure.Field.TIMESTAMP.getFieldName())));
			
			JsonObject jsonValue = JsonObject.mapFrom(value.getJsonObject(ElasticSearchIndexerConstants.Message.Structure.Field.MESSAGE.getFieldName()));
			
			jsonValue.put(ElasticSearchIndexerConstants.Message.Structure.Field.TIMESTAMP.getFieldName(), indexTimeStampPattern.format(value.getJsonObject(ElasticSearchIndexerConstants.Message.Structure.Field.META.getFieldName()).getLong(ElasticSearchIndexerConstants.Message.Structure.Field.TIMESTAMP.getFieldName())));
